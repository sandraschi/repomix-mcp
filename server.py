...
class PackRepositoryRequest(BaseModel):
    ...
    output_file: Optional[str] = Field(None, description="Output file path (optional, defaults to repomix-output.xml)")
    style: Optional[str] = Field("xml", description="Output format: xml, markdown, json, plain")
    include_patterns: Optional[List[str]] = Field(None, description="Glob patterns to include")
    ignore_patterns: Optional[List[str]] = Field(None, description="Glob patterns to ignore")
    compress: Optional[bool] = Field(False, description="Use Tree-sitter compression for ~70% token reduction")
...
class ReadPackedOutputRequest(BaseModel):
    ...
    packed_file_path: str = Field(..., description="Path to the packed output file")
    max_lines: Optional[int] = Field(100, description="Maximum lines to return")
...
async def pack_repository(request: PackRepositoryRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            return {
                "success": False,
                "error": "Repomix is not installed. Install with: npm install -g repomix"
            }
...
async def read_packed_output(request: ReadPackedOutputRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        if not packed_file.exists():
            return {
                "success": False,
                "error": f"Packed file does not exist: {packed_file}"
            }

        # Read file content
        with open(packed_file, 'r', encoding='utf-8', errors='replace') as f:
            content = f.read()
        ...
        return {
            "success": False,
            "error": f"Failed to read packed output: {str(e)}"
        }
...
async def search_packed_output(request: SearchPackedOutputRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": False,
            "error": f"Failed to search packed output: {str(e)}"
        }
...
if __name__ == "__main__":
    # Run the MCP server
    mcp.run()  # This will handle stdio communication with the MCP client