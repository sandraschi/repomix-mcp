...
const STORAGE_KEY = "repomix-mcp-chat-history";

const PERSONALITIES: Record<string, string> = {
  "Code Analyst": "You are an expert code analyst. Help users understand codebases, review code quality, identify patterns, and suggest improvements. Be thorough and reference specific files and functions.",
  "Project Architect": "You are a seasoned software architect. Advise on project structure, dependency management, module organization, and design patterns. Think about the big picture and long-term maintainability.",
  "Quick Summarizer": "You are a concise assistant. Answer in 1-3 sentences. Be direct and to the point.",
  "Custom": "",
};

const EXAMPLE_PROMPTS = [
  { group: "Analysis", items: ["Analyze the dependency graph of this project", "Find unused functions and dead code paths", "Review code complexity and suggest refactoring"] },
  { group: "Structure", items: ["Map the project module architecture", "Compare directory structure against best practices", "Suggest a better package organization"] },
  { group: "Dependencies", items: ["List all npm dependencies and their licenses", "Check for outdated or vulnerable packages", "Generate a dependency tree visualization"] },
];
...
export function Chat() {
  const [messages, setMessages] = useState<Message[]>(() => {
    ...
    return [{
      role: "assistant",
      content: "I'm your RepoMix MCP assistant. I can analyze codebases, map project structures, review dependencies, and suggest improvements. What repository would you like me to analyze?",
      timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
    }];
  });
  const [inputValue, setInputValue] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const [personality, setPersonality] = useState("Code Analyst");
  ...
  const handleClear = () => {
    const fresh: Message[] = [{
      role: "assistant",
      content: "Conversation cleared. Ready to analyze a new repository.",
      timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
    }];
    setMessages(fresh);
    saveMessages(fresh);
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `repomix-mcp-chat-${new Date().toISOString().split("T")[0]}.txt`;
    a.click();
    ...
    <div className="flex flex-col space-y-4 animate-in fade-in duration-500 h-full" data-testid="chat-page">
      <div className="flex items-center justify-between shrink-0" data-testid="chat-controls">
        <div className="flex items-center gap-4">
          ...
          <span className="text-xs text-muted-foreground bg-muted/30 px-2 py-0.5 rounded font-mono">skill:code-analyst</span>
        </div>
        <div className="flex items-center gap-2">
        ...
        <div className="p-4 border-t border-border/30 bg-secondary/10 shrink-0">
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="flex gap-3">
            <input ref={inputRef}
              ...
              placeholder="Ask about repository analysis..."
              value={inputValue}
